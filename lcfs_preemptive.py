...
for utilisation in range(0,len(rho)):
     ...
     for iterate in range(0,Monte_Carlo):
          ...
          for i in range(0,len(arrival)):
               leaving_time.append(0);
          
          #initializing the list leaving_time
          leaving_time[0]=arrival[0]+service_duration[0];
          ...
          #Variable to store the index of packet which is preempted from service
          pointer=1;
          
          while(np.max(leaving_time)<arrival[-1]):
               
               counter=0;
               
               while(arrival[counter]<np.max(leaving_time)):
                    counter=counter+1;
                    
               if(index==counter-1):
                    leaving_time[pointer]=arrival[counter]+service_duration[counter];
                    pointer=pointer+1;
                    index=counter;
                    
               else:
                    leaving_time[pointer]=arrival[counter-1]+service_duration[counter-1];
                    leaving_time[pointer-1]=arrival[index]+service_duration[index]+service_duration[counter-1];
                    pointer=pointer+1;
                    index=counter-1;
